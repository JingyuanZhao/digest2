...
class Digest2GUI:
    ...
    def create_description_tab(self):
        ...
        tree.heading('fullname', text='英文全称')
        tree.heading('chinese', text='中文含义')
        tree.heading('definition', text='Digest2定义')
        
        # 设置列宽（全部左对齐）
        tree.column('abbr', width=60, anchor=tk.W)
        tree.column('fullname', width=240, anchor=tk.W)
        tree.column('chinese', width=110, anchor=tk.W)
        tree.column('definition', width=760, anchor=tk.W)
        ...
        orbit_types = [
            ...
            ('N22', 'Intermediate-size Near-Earth Object', '中等大小近地天体', '直径大于140米。近日点 q < 1.3 AU，绝对星等 H < 22.5'),
            ('N18', 'Large Near-Earth Object', '大型近地天体', '直径大于1.2千米。近日点 q < 1.3 AU，绝对星等 H < 18.5'),
        ...
        self.desc_menu.add_command(label="复制选中行", command=self.copy_desc_selected)
        self.desc_menu.add_command(label="复制全部数据", command=self.copy_desc_all)
        
        # 绑定右键点击事件
        tree.bind('<Button-3>', self.show_desc_menu)
        # 绑定左键点击事件，实现点击切换选中
        tree.bind('<Button-1>', self.on_desc_tree_click)
    ...
    def show_desc_menu(self, event):
        """显示类型说明表格的右键菜单"""
        try:
            self.desc_menu.tk_popup(event.x_root, event.y_root)
        finally:
            self.desc_menu.grab_release()
    ...
    def copy_desc_all(self):
        """复制类型说明表格中的全部数据"""
        data = []
        # 添加表头
        data.append('\t'.join(['缩写', '英文全称', '中文含义', 'Digest2定义']))
